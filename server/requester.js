...
var Requester = function (key, secret) {
  ...
  this.apiAllocation = {
    explanation: "http://developer.factual.com/throttling-limits/"
  };
};

Requester.prototype = {
  getApiAllocation: function () {
    return this.apiAllocation;
  },
  ...
  getCallback: function (req) {
    ...
    return function (error, data, response) {
      ...
      if (req.url.indexOf('/places') > -1) {
        me.apiAllocation.places = JSON.parse(response.headers['x-factual-throttle-allocation']);
        me.apiAllocation.lastUpdated = '' + new Date();
      } else {
        if (req.url.indexOf('/crosswalk') > -1) {
          me.apiAllocation.crosswalk = JSON.parse(response.headers['x-factual-throttle-allocation']);
          me.apiAllocation.lastUpdated = '' + new Date();
        }
      }
